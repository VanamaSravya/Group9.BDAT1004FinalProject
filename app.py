...
app = Flask(__name__)
api = Api(app)
model = pickle.load(open('ck_model.pkl', 'rb'))
...
def generate_data_set(url):
    ...
    known_fake = ['become007', 'dentaldisinfection', 'Bazaarcart', 'ogool']

    for fake in known_fake:
        if fake in url:
            print('Fake')
            return None
...
def verify(url):
    print(url)
    url_features = generate_data_set(url)
    # print(url_features)

    try:
        if url_features:
            x = model.predict([url_features])
            prediction = x[0]
...
def predict():
    ...
    #
    # print(int_features)
    # inputs = [np.array(int_features)]
    # prediction = model.predict(inputs)
    url = request.form['sub']
    ans = verify(url)
    return render_template('index.html', status=ans)


api.add_resource(VerifyAPI, '/status/<path:url>')
